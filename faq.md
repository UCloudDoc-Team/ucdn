...
A：支持动态加速以及全站加速，针对静态需求缓存，动态请求优化选路回源。

### Q：是否支持泛域名加速

A：支持泛域名加速：使用通配符做加速域名，用来表示所有的次级域名的加速；例如在UCDN配置的加速域名为 “.ucloud.com”,则对应的次级域名aaa.ucloud.com,bbb.ucloud.com 都可以解析到UCDN对应的泛域名.ucloud.com 的CNAME进行加速。
...
A：一般看用户数据的热点。文件的访问量越大，命中率相应的就会越高；正常情况下下载业务命中率较高，在95%以上，点播业务在90%左右，网页文件命中率较低，且有部分动态资源，命中率根据其网站性质相关，基本在75%左右。

命中率较低主要排查以下情况：

* 网站访问量较低，文件热度不够，CDN节点收到的请求较少，有效缓存的文件就比较少，导致用户的请求无法在节点命中，影响命中率较低。
* 加速域名的缓存规则配置不合理，针对某些文件设置缓存时长较短或者不缓存，导致CDN节点频繁回源，命中率降低
* 源站的缓存 HTTP Header设置不当，Header中包含no-cache、cache-control=0等，会被CDN当做最高优先级执行不缓存。
* 源站动态资源较多，多为不可缓存的内容，也会导致频繁回源拉取。

### Q： 如何获取最终用户IP？

A：正式切换使用CDN服务后，请求通过边缘加速节点，会默认增加 x-forward-for 头部，携带客户端真实 IP 信息,日志也会保存请求的用户IP。
...
### Q：实时监控拉取到的带宽数据和流量数据值是怎么来的？

A：实时监控展示账号下加速域名的流量和带宽数据来源于所有节点的下行带宽/流量，不包括回源带宽/流量。

### Q：实时监控打点带宽、流量、请求数、命中率数据是怎么计算来的？

A：带宽统计是按照1分钟（最小展示单位）的峰值加起来/时间粒度的平均值统计的，比如时间粒度是5分钟，带宽值就是5个1分钟的带宽峰值相加/5 ；流量和请求数数据是每分钟加起来的数据；命中率=每个时间间隔的字节命中百分占比。

### Q: 实时监控数据延时大概有多久？

A：国内数据延时在5-10分钟左右，海外数据延时在15-25分钟左右。
...
A： 1.打开hosts文件(windows存放在：C:\Windows\System32\drivers\etc 目录下,OS系统
...
### Q：为什么停止UCDN域名加速，仍然会产生带宽和费用。

A：用户local DNS仍存在DNS解析缓存，在缓存未过期期间，会请求到对应的节点上，产生少量带宽流量，DNS解析缓存时间由各个地区运营商控制。
