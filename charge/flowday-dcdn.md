...
* 全站加速境内目前支持的计费方式有预付费流量包、带宽峰值日付、月付带宽、月付流量；境外目前支持的计费方式有预付费流量包、月付流量。
...
| ------------- | ---------- | ------------- |
| 1GB~10TB      | 0.22元/GB   | 0.45元/GB      |
| 10TB(含)~50TB  | 0.20元/GB   | 0.40元/GB      |
| 50TB(含)~100TB | 0.18元/GB   | 0.36元/GB      |
| 100TB(含)~1PB  | 0.16元/GB   | 0.32元/GB      |
| ≥1PB           | 0.15元/GB   | 0.32元/GB      |  

ps：前闭后开，例如用户购买100TB的全站加速流量包,则对应消费为0.16x100x1024=16384元
...
全站加速域名根据请求类型及所属区域收取请求数费用（非HTTP协议的静态请求及所有动态请求均收费）。海外全站加速请求数计费包含http/https/quic/websocket协议。境内websocket单独计费，[计费参考](/ucdn/charge/month?id=全站加速websocket月流量计费)。
